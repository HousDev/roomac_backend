...
const MaterialPurchaseModel = {
  ...
  getAllPurchases: async (filters = {}) => {
    try {
      ...
      // Parse items JSON safely – handle both string and object
      rows.forEach((row) => {
        if (row.items) {
          try {
            // If items is already an object (MySQL JSON column auto-parsed), use directly
            if (typeof row.items === "string") {
              row.purchase_items = JSON.parse(row.items);
            } else {
              // Already an object, assign as is
              row.purchase_items = row.items;
            }
          } catch (e) {
            console.error(
              "Error parsing items JSON for row ID",
              row.id,
              ":",
              e,
            );
        ...
        // DO NOT delete row.items – frontend may need the raw JSON string
      });
      ...
      if (rows.length > 0) {
        ...
        if (row.items) {
          try {
            ...
            console.error("Error parsing items JSON for ID", id, ":", e);
            row.purchase_items = [];
          }
        } else {
          row.purchase_items = [];
        }
        // Keep original items for compatibility if needed, but frontend uses purchase_items
        // Optionally delete row.items to avoid confusion
        // delete row.items;
        return row;
      }
      return null;
  ...
  createPurchase: async (data) => {
    try {
      ...
      // Convert items array to JSON string (always store as string in DB)
      let itemsJson;
      try {
        itemsJson = JSON.stringify(items || []);
      } catch (e) {
  ...
  updatePurchase: async (id, updateData) => {
    try {
      ...
      if (updateData.items) {
        try {
          // If items is an array, stringify; if it's already a string, keep as is
      ...
      if (
        ...
        if (existing) {
          ...
          const paid =
            updateData.paid_amount !== undefined
              ? parseFloat(updateData.paid_amount)
  ...
  // Add payment (update purchase with payment info)
  addPayment: async (id, paymentData) => {
    try {
      const {
        payment_date,
  ...
  deletePurchase: async (id) => {
    try {
      const [result] = await db.query(
        "DELETE FROM material_purchases WHERE id = ?",
        [id],
      );
      return result;
    } catch (err) {
