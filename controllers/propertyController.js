...
//  * Handles both FormData and JSON formats
//  */
...
//  */
// function buildPhotoURLs(req, existingPhotos = [], removedPhotos = []) {
//   const files = req.files || [];
//   const newPhotos = Array.isArray(files)
//     ? files.map((f) => `/uploads/properties/${f.filename}`)
//     : [];

//   // Filter out removed photos from existing ones
//   const filteredExisting = Array.isArray(existingPhotos)
//     ? existingPhotos.filter((photo) => !removedPhotos.includes(photo))
//     : [];

//   return [...filteredExisting, ...newPhotos];
// }
...
//       // Prepare update data - Start with photo_urls
//       const updateData = {
//         photo_urls: finalPhotoUrls, // This will always be updated
//       };
...
//       const properties = await PropertyModel.findByIds(ids);
...
//       await PropertyModel.bulkDelete(ids);

//       return res.json({
//         success: true,
//         message: `${ids.length} properties deleted`,
//       });
//     } catch (error) {
...
//     return res.json({
//       success: true,
//       message: `${ids.length} properties updated to ${is_active ? 'active' : 'inactive'}`,
